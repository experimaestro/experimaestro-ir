...
@argument("model")
@config(NS.capreolus.model)
def CapreolusModel():
    def create(self):
        module = importlib.import_module(f"capreolus.reranker.reranker.{self.CAPREOLUS_NAME}")
...
@argument("nodes", default=5, help="hidden layer dimension for feed forward matching network")
...
@config(NS.capreolus.model.drmm)
class DRMM(CapreolusModel): 
    CAPREOLUS_NAME = "DRMM"



@argument("maxdoclen", default=800, help="maximum document length (in number of terms after tokenization)")
@argument("maxqlen", default=4, help="maximum query length (in number of terms after tokenization)")
@argument("batch", default=32, help="batch size")
@argument("niters", default=150, help="number of iterations to train for")
@argument("itersize", default=4096, help="number of training instances in one iteration (epoch)")
@argument("gradacc", default=1, help="number of batches to accumulate over before updating weights")
...
@argument("softmaxloss", default=True, help="True to use softmax loss (over pairs) or False to use hinge loss")
@config()
class Pipeline(): pass
...
@argument("fold", type=int, help="Fold for learning")
@argument("reranker", type=CapreolusModel)

# From pipeline config

def ModelLearn(dataparallel, fold, reranker):
    # Adapted from train.py#train + pi
