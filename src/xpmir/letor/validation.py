...
class AggregatorValidationListener(LearnerListener):
    ...
    def __validate__(self):
        ...
        if len(intervals) != 1:
            raise ValueError(
                f"Listeners have different validation intervals: {intervals}"
            )

        assert self.validation_interval == intervals.pop(), (
            "The validation interval of the aggregator should be the same as "
            "the listeners"
        )

        # Check that the metrics are the same
        key_sets = [set(listener.metrics.keys()) for listener in self.listeners]
        ...
        for i, ks in enumerate(key_sets[1:], start=1):
            if ks != ref_keys:
                raise ValueError(
                    f"Metric key mismatch between listeners:\n"
