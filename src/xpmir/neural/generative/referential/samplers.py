...
# FIXME: Store the external ids in the matrix by using numpy instead of internal ids!
...
class DynamicNegativesUpdatableSampler(NegativesUpdatableSampler):
    ...
    log_proba_mean_matrix: torch.Tensor
    """A matrix of shape [dim, dim, ..., dim, N_2] of dim max_depth + 1, which
    contains the id of the documents
    ...
    def __post_init__(self):
        ...
        # initialize the ids with -1.
        self.id_matrix = torch.ones(shape_ids, dtype=torch.int32).detach() * (-1)
        # initialize the log_probabilities with the a small probability
        self.log_proba_mean_matrix = torch.full(shape_proba, -10.0).detach()

    def update_matrix(
        self,
        sampled_tokens: torch.Tensor,  # shape [max_depth, bs]
        ids: torch.Tensor,  # shape: bs
        log_proba: torch.Tensor,  # shape: bs
    ):
        with torch.no_grad():
