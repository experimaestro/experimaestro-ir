...
def test_serializing_tripletbasedsampler():
    ...
    for _, _ in zip(range(10), sampler.pairwiserecord_iter()):
        pass

    data = sampler.state_dict()
    samples = []
    for _, record in zip(range(10), sampler.pairwiserecord_iter()):
    ...
    for _, record, expected in zip(range(10), sampler.pairwiserecord_iter(), samples):
